...
def run_classify(lr=0.001,
    ...
    class Model(torch.nn.Module):
        ...
        def forward(self, x, with_att=False):
            ...
            _, xenc2 = self.enc2(_xemb, gate=xatt, mask=xmask[:, :xenc.size(1)], ret_states=True)
...
class EncoderSummarizer(q.att.AttentionSummarizer):
    def __init__(self, summ_core=None, **kw):
        super(EncoderSummarizer, self).__init__(**kw)
        self.core = summ_core

    def summarize(self, alphas, values):
        y, y_T = self.core(values, mask=alphas)
        return y_T


class Attention(q.att.SigmoidNormalizer, EncoderSummarizer, q.att.DotScoreComputer, q.att.Attention): pass


def run_decode(lr=0.001,
        seqlen=6,
        numex=500,
        epochs=25,
        batsize=10,
    ...
    # encoders:
    enc1 = q.RNNEncoder(embdim, encdim, bidir=True)
    enc2 = q.RNNCellEncoder(enc2inpdim, outdim, bidir=False)
    att = Attention(summ_core=enc2)
    ...
    class Model(torch.nn.Module):
        ...
        def forward(self, x, with_att=False):
            ...
            _xemb = self.emb_expander(xemb[:, :xenc.size(1)])
            _xenc = self.enc_expander(xenc)
            _, xenc2 = self.enc2(_xemb+_xenc, gate=xatt, mask=xmask[:, :xenc.size(1)])
