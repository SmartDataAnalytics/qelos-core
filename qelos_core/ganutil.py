# direct copy from https://github.com/Cadene/pretrained-models.pytorch/blob/master/pretrainedmodels/models/inceptionresnetv2.py
...
class InceptionResNetV2(nn.Module):
    ...
    def logits(self, features):
        ...
        x = self.last_linear(activations)
        return x, activations

    def forward(self, input, with_activations=False):
        x = self.features(input)
        x, activations = self.logits(x)
        if with_activations:
            return x, activations
        else:
            return x
